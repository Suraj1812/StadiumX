...
export function HighlightsGallery() {
  return (
    <SectionShell
      ...
      className="overflow-hidden bg-[linear-gradient(180deg,#050505,#071B2A_45%,#050505)]"
    >
        ...
        {gallery.map((item, index) => (
          <Reveal key={item.title} delay={index * 0.06} className={cn(item.span)}>
            <motion.article
              ...
              className="group relative h-full overflow-hidden rounded-md border border-white/10 bg-[#07110e]"
            >
              <Image
                src={item.image}
                alt={item.title}
              ...
              <div className="absolute left-4 top-4 flex items-center gap-2 rounded-full border border-white/15 bg-black/45 px-3 py-1 text-[0.65rem] font-black uppercase tracking-[0.18em] text-white/70 backdrop-blur-md">
                <ScanLine className="h-3.5 w-3.5 text-[#5CFF8F]" />
                {item.kicker}
              </div>
              <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-4">
                <h3 className="font-display text-4xl uppercase leading-none text-white">{item.title}</h3>
                <span className="grid h-12 w-12 shrink-0 place-items-center rounded-full border border-[#5CFF8F]/50 bg-[#5CFF8F]/15 text-[#5CFF8F] shadow-neon-ring transition group-hover:bg-[#5CFF8F] group-hover:text-[#04130d]">
